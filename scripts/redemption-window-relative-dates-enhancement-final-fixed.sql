-- Redemption Window Relative Dates Enhancement - FINAL FIXED VERSION
-- Date: August 23, 2025
-- Adds alternative date configuration options for redemption windows
-- FIXES: Column default casting error AND constraint IF NOT EXISTS syntax error
...
DO $$
BEGIN
    -- Add lockup_days constraint if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add processing_offset_days constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'chk_processing_offset_days_non_negative' 
        AND table_name = 'redemption_windows'
    ) THEN
        ALTER TABLE redemption_windows 
