...
-- FIXED VERSION: Removed IMMUTABLE function constraint issues
...
-- Create indexes for performance (FIXED: Removed IMMUTABLE constraint issues)
...
-- Create index for active calculations (standard index without time predicate)
...
-- Create RLS policies
-- Users can see risk calculations for receivables they have access to
CREATE POLICY "Users can view risk calculations for accessible receivables" ON public.climate_risk_calculations
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.climate_receivables cr
            WHERE cr.receivable_id = climate_risk_calculations.receivable_id
            AND cr.created_by = auth.uid()
        )
    );

-- Users can insert risk calculations for receivables they created
CREATE POLICY "Users can create risk calculations for their receivables" ON public.climate_risk_calculations
    FOR INSERT
    WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.climate_receivables cr
...
-- Users can update risk calculations for their receivables
CREATE POLICY "Users can update risk calculations for their receivables" ON public.climate_risk_calculations
    FOR UPDATE
    USING (
        EXISTS (
            SELECT 1 FROM public.climate_receivables cr
...
-- Add comment
COMMENT ON TABLE public.climate_risk_calculations IS 'Automated risk calculation results for climate receivables including production, credit, and policy risk components';
