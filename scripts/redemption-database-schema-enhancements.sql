-- Redemption Database Schema Enhancements
-- Date: August 26, 2025
...
ALTER TABLE public.redemption_windows 
ADD COLUMN IF NOT EXISTS submission_status TEXT DEFAULT 'not_started' CHECK (
    submission_status IN ('not_started', 'open', 'closed', 'extended', 'cancelled')
),
ADD COLUMN IF NOT EXISTS processing_status TEXT DEFAULT 'pending' CHECK (
...
-- Add foreign key relationships for audit trail
ALTER TABLE public.redemption_windows 
ADD CONSTRAINT IF NOT EXISTS fk_redemption_windows_last_modified_by 
    FOREIGN KEY (last_modified_by) REFERENCES auth.users (id),
ADD CONSTRAINT IF NOT EXISTS fk_redemption_windows_approved_by 
...
ADD COLUMN IF NOT EXISTS compliance_status TEXT DEFAULT 'pending' CHECK (
    compliance_status IN ('pending', 'approved', 'rejected', 'requires_review')
);
...
ALTER TABLE public.redemption_requests 
ADD CONSTRAINT IF NOT EXISTS fk_redemption_requests_window 
    FOREIGN KEY (redemption_window_id) REFERENCES redemption_windows (id) ON DELETE CASCADE,
ADD CONSTRAINT IF NOT EXISTS fk_redemption_requests_requested_by 
    FOREIGN KEY (requested_by) REFERENCES auth.users (id),
ADD CONSTRAINT IF NOT EXISTS fk_redemption_requests_approved_by 
    FOREIGN KEY (approved_by) REFERENCES auth.users (id),
ADD CONSTRAINT IF NOT EXISTS fk_redemption_requests_processed_by 
...
CREATE TABLE IF NOT EXISTS public.redemption_window_templates (
    ...
    submission_date_mode PUBLIC.submission_date_mode_enum NOT NULL DEFAULT 'fixed',
    processing_date_mode PUBLIC.processing_date_mode_enum NOT NULL DEFAULT 'fixed',
    ...
    -- Constraints
    CONSTRAINT fk_templates_created_by FOREIGN KEY (created_by) REFERENCES auth.users (id),
    CONSTRAINT fk_templates_project FOREIGN KEY (project_id) REFERENCES projects (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.redemption_notifications (
    ...
    notification_type TEXT NOT NULL CHECK (
        notification_type IN ('window_opening', 'window_closing', 'request_approved', 
    ...
    CONSTRAINT fk_notifications_window FOREIGN KEY (redemption_window_id) REFERENCES redemption_windows (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.redemption_analytics (
    ...
    CONSTRAINT fk_analytics_window FOREIGN KEY (redemption_window_id) REFERENCES redemption_windows (id) ON DELETE CASCADE
);
...
BEGIN
    ...
    IF OLD.status != NEW.status OR OLD.submission_status != NEW.submission_status OR OLD.processing_status != NEW.processing_status THEN
...
CREATE OR REPLACE FUNCTION update_redemption_analytics()
RETURNS TRIGGER AS $$
DECLARE
    window_id UUID;
    analytics_record RECORD;
BEGIN
    -- Determine window_id based on operation
    IF TG_OP = 'DELETE' THEN
        window_id := OLD.redemption_window_id;
    ELSE
        window_id := NEW.redemption_window_id;
    END IF;
    
    -- Skip if no window_id
    ...
    -- Recalculate analytics for the window
    INSERT INTO public.redemption_analytics (
        redemption_window_id,
        total_requested_tokens,
        total_approved_tokens,
    ...
    SELECT 
        ...
        COALESCE(SUM(CASE WHEN status = 'approved' THEN token_amount ELSE 0 END), 0),
        COUNT(DISTINCT investor_id),
        COALESCE(AVG(token_amount), 0)
    FROM public.redemption_requests 
    WHERE redemption_window_id = window_id
    ON CONFLICT (redemption_window_id) 
    DO UPDATE SET
        total_requested_tokens = EXCLUDED.total_requested_tokens,
        total_approved_tokens = EXCLUDED.total_approved_tokens,
        participating_investors = EXCLUDED.participating_investors,
        average_request_size = EXCLUDED.average_request_size,
        calculated_at = now();
...
DROP TRIGGER IF EXISTS tr_redemption_requests_analytics ON public.redemption_requests;
CREATE TRIGGER tr_redemption_requests_analytics
    AFTER INSERT OR UPDATE OR DELETE ON public.redemption_requests
    FOR EACH ROW EXECUTE FUNCTION update_redemption_analytics();
