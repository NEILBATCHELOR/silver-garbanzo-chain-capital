...
-- Add new columns to support relative date configurations
ALTER TABLE redemption_windows 
ADD COLUMN IF NOT EXISTS submission_date_mode VARCHAR(20) DEFAULT 'fixed',
ADD COLUMN IF NOT EXISTS processing_date_mode VARCHAR(20) DEFAULT 'fixed',
ADD COLUMN IF NOT EXISTS lockup_days INTEGER,
ADD COLUMN IF NOT EXISTS processing_offset_days INTEGER DEFAULT 1;
...
CREATE TYPE IF NOT EXISTS submission_date_mode_enum AS ENUM ('fixed', 'relative');
CREATE TYPE IF NOT EXISTS processing_date_mode_enum AS ENUM ('fixed', 'same_day', 'offset');
...
ADD CONSTRAINT chk_processing_offset_days_non_negative CHECK (processing_offset_days >= 0);
...
-- Sample data update - convert existing windows to use 'fixed' mode
...
-- Add example data showing new functionality
...
INSERT INTO redemption_windows (
    ...
    uuid_generate_v4(), 
    (SELECT project_id FROM redemption_windows LIMIT 1), -- Use existing project
    uuid_generate_v4(),
    'relative', 'offset',
