...
DELETE FROM token_allocations WHERE subscription_id IN (SELECT id FROM subscriptions WHERE project_id = 'test-project-1');
DELETE FROM subscriptions WHERE project_id = 'test-project-1';
DELETE FROM cap_table_investors WHERE cap_table_id IN (SELECT id FROM cap_tables WHERE project_id = 'test-project-1');
DELETE FROM cap_tables WHERE project_id = 'test-project-1';
DELETE FROM projects WHERE id = 'test-project-1';
DELETE FROM investors WHERE investor_id LIKE 'test-investor-%';
...
VALUES ('test-project-1', 'Test Project 1', 'Dashboard metrics test project', 'active', 'equity', 'TST1', 5000000, 1000000, 5.00, NOW(), NOW());

-- Create a cap table
INSERT INTO cap_tables (id, project_id, name, description, created_at, updated_at)
VALUES ('test-cap-table-1', 'test-project-1', 'Test Cap Table', 'Cap table for test project', NOW(), NOW());
