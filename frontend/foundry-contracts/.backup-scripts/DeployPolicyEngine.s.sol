...
contract DeployPolicyEngine is Script {
    ...
    function run() external returns (address policyEngine, address token) {
        ...
        // Create TokenConfig struct for EnhancedERC20Token
        EnhancedERC20Token.TokenConfig memory tokenConfig = EnhancedERC20Token.TokenConfig({
            name: "Chain Capital Token",
            symbol: "CCT",
            decimals: 18,
            initialSupply: 1000000 * 10**18,
            maxSupply: 10000000 * 10**18,
            initialOwner: config.admin,
            policyEngineAddress: policyEngine,
            mintingEnabled: true,
            burningEnabled: true,
            pausable: true,
            votingEnabled: true,
        ...
        token = address(new EnhancedERC20Token(tokenConfig));
