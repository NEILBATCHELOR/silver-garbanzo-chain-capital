...
library LiquidationLogic {
  ...
  function executeEliminateDeficit(
    ...
    bool isActive = reserveCache.commodityConfiguration.getActive();
    ...
    uint256 userBalance = reserveCache.commodityConfiguration.getIsVirtualAccActive()
      ? IAToken(reserveCache.cTokenAddress).scaledBalanceOf(msg.sender).rayMul(
  ...
  function executeLiquidationCall(
    ...
    DataTypes.CommodityReserveData storage collateralReserve = reservesData[params.collateralAsset];
    DataTypes.CommodityReserveData storage debtReserve = reservesData[params.debtAsset];
    ...
    vars.collateralAToken = IAToken(collateralReserve.cTokenAddress);
    ...
    vars.debtAssetUnit = 10 ** vars.debtReserveCache.commodityConfiguration.getDecimals();
    ...
    IERC20(params.debtAsset).safeTransferFrom(
      msg.sender,
      vars.debtReserveCache.cTokenAddress,
      vars.actualDebtToLiquidate
    );

    IAToken(vars.debtReserveCache.cTokenAddress).handleRepayment(
      msg.sender,
      params.user,
      vars.actualDebtToLiquidate
    );
  ...
  function _burnDebtTokens(
    ...
    if (hasNoCollateralLeft && outstandingDebt != 0) {
       ...
       */
      if (!debtReserveCache.commodityConfiguration.getIsVirtualAccActive()) {
        ...
        if (accruedInterest != 0 && accruedInterest > actualDebtToLiquidate) {
          ...
          IAToken(debtReserveCache.cTokenAddress).handleRepayment(address(0), user, amountToBurn);
        }
      }
  ...
  function _calculateAvailableCollateralToLiquidate(
    DataTypes.CommodityConfigurationMap memory collateralReserveConfiguration,
    uint256 collateralAssetPrice,
    uint256 collateralAssetUnit,
    uint256 debtAssetPrice,
    uint256 debtAssetUnit,
  ...
  function _burnBadDebt(
    ...
    for (uint256 i; i < reservesCount; i++) {
      ...
      DataTypes.CommodityReserveData storage currentReserve = reservesData[reserveAddress];
      DataTypes.ReserveCache memory reserveCache = currentReserve.cache();
      if (!reserveCache.commodityConfiguration.getActive()) continue;
