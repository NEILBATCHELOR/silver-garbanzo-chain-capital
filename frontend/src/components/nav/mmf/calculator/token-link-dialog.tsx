/**
 * MMF Token Link Dialog
 * Dialog for creating/editing MMF-token links with parity ratios
 */
...
export function TokenLinkDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['mmfs'] })
      onSuccess?.()
      onOpenChange(false)
    },
    onError: (error: Error) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Link <strong>{fundName}</strong> to a token and set the parity ratio
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Select the token that will be backed by this MMF
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Ratio of MMF shares to token units (1.0 = 1:1 parity)
            </p>
          </div>
          ...
          {parityRatio && collateralizationPercentage && !isNaN(parseFloat(parityRatio)) && !isNaN(parseFloat(collateralizationPercentage)) && (
            <div className="p-3 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-md">
              ...
              <p className="text-xs text-blue-700 dark:text-blue-300">
                If MMF NAV = $1.00 per share, then token value = ${((1.0 * parseFloat(parityRatio)) * (parseFloat(collateralizationPercentage) / 100)).toFixed(4)}
              </p>
            </div>
          )}
        </div>
