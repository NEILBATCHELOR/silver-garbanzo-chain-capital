...
interface BulkIncentivesUploadProps {
  projectId: string;
  onUploadComplete?: (data: any[]) => void;
  onCancel?: () => void;
}

const BulkIncentivesUpload = ({
  projectId,
  onUploadComplete = () => {},
  onCancel = () => {},
}: BulkIncentivesUploadProps) => {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < totalBatches; i++) {
        ...
        const { data, error } = await supabase.from('climate_incentives').insert(batch).select();
  ...
  const downloadTemplate = () => {
    const sampleData = [
      {
        ...
        receivable_id: "00000000-0000-0000-0000-000000000004",
        type: "Tax Credit",
        amount: 10000,
        status: "pending",
    ...
    link.setAttribute("download", "incentives_template.csv");
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Bulk Incentives Upload</CardTitle>
        <CardDescription>
          Upload a CSV file with climate incentives (including RECs).
        </CardDescription>
      </CardHeader>
...
export default BulkIncentivesUpload;
