...
interface BulkProductionDataUploadProps {
  projectId: string;
  onUploadComplete?: (data: any[]) => void;
  onCancel?: () => void;
}

const BulkProductionDataUpload = ({
  projectId,
  onUploadComplete = () => {},
  onCancel = () => {},
}: BulkProductionDataUploadProps) => {
  ...
  const handleUpload = async () => {
    ...
    const recordsToInsert = parsedData.map(row => ({
        ...
        output_mwh: row.output_mwh,
        weather_condition_id: row.weather_condition_id,
    }));
    ...
    for (let i = 0; i < totalBatches; i++) {
        ...
        const { data, error } = await supabase.from('production_data').insert(batch).select();
  ...
  const downloadTemplate = () => {
    const sampleData = [
      {
        ...
        output_mwh: 150.5,
        weather_condition_id: "00000000-0000-0000-0000-000000000002",
      },
    ];
    ...
    link.setAttribute("download", "production_data_template.csv");
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Bulk Production Data Upload</CardTitle>
        <CardDescription>
...
export default BulkProductionDataUpload;
