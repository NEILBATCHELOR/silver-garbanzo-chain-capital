...
import { ClimateTokenAllocation } from "../hooks/useClimateTokenDistribution";
import { TokenStandard } from "@/types/core/centralModels";

interface ClimateBulkEditAllocationsProps {
  selectedAllocations: ClimateTokenAllocation[];
  onRefresh: () => void;
  onDeselectAll: () => void;
}

export const ClimateBulkEditAllocations: React.FC<ClimateBulkEditAllocationsProps> = ({
  selectedAllocations,
  onRefresh,
  onDeselectAll,
}) => {
  ...
  // Handle bulk delete of climate token allocations
  const handleBulkDelete = async () => {
    try {
      ...
      if (distributedAllocations.length > 0) {
        setError(
          `Cannot delete ${distributedAllocations.length} climate allocation(s) that have already been distributed.`
        );
        return;
      }
      ...
      toast({
        title: "Success",
        description: `Successfully deleted ${allocationIds.length} climate allocation(s)`,
      });
      ...
      console.error("Error deleting climate allocations:", error);
  ...
  // Handle bulk edit of climate token allocations
  const handleBulkEdit = async () => {
    try {
      ...
      if (distributedAllocations.length > 0 && !allowedFieldsForDistributed.includes(editField)) {
        setError(
          `Cannot modify ${distributedAllocations.length} climate allocation(s) that have already been distributed. You can only update notes, distribution status, token standard, or symbol for distributed allocations.`
        );
        return;
      }
      ...
      toast({
        title: "Success",
        description: `Successfully updated ${allocationIds.length} climate allocation(s)`,
      });
      ...
      console.error("Error updating climate allocations:", error);
      setError(error instanceof Error ? error.message : String(error));
      toast({
        title: "Error",
        description: "Failed to update climate allocations",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirm Climate Allocation Deletion</DialogTitle>
            <DialogDescription>
              Are you sure you want to delete {selectedAllocations.length} selected climate allocation(s)?
          ...
          <div className="space-y-4 py-4">
            <div>
              <h3 className="font-medium">Selected Climate Allocations:</h3>
              <div className="mt-2 flex flex-wrap gap-2">
                {selectedAllocations.map((allocation) => (
                  <Badge key={allocation.id} variant={allocation.distributionStatus === 'completed' ? "default" : "outline"}>
                    {allocation.investorName} - {allocation.tokenAmount.toLocaleString()} climate tokens
                  </Badge>
                ))}
              </div>
            </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Bulk Edit Climate Allocations</DialogTitle>
            <DialogDescription>
              Edit {selectedAllocations.length} selected climate allocation(s).
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Select value={editField} onValueChange={setEditField}>
                ...
                <SelectContent>
                  <SelectItem value="tokenAmount">Climate Token Amount</SelectItem>
              ...
              <div>
                ...
                {editField === "distributionStatus" ? (
                  ...
                  <Input
                    ...
                    placeholder={
                      editField === "tokenAmount" ? "Enter climate token amount" : 
...
export default ClimateBulkEditAllocations;
