...
interface BulkCarbonOffsetsUploadProps {
  projectId: string;
  onUploadComplete?: (data: any[]) => void;
  onCancel?: () => void;
}

const BulkCarbonOffsetsUpload = ({
  projectId,
  onUploadComplete = () => {},
  onCancel = () => {},
}: BulkCarbonOffsetsUploadProps) => {
  ...
  const handleUpload = async () => {
    ...
    const recordsToInsert = parsedData.map(row => ({
        ...
        vintage: row.vintage,
        quantity_tco2e: row.quantity_tco2e,
    ...
    for (let i = 0; i < totalBatches; i++) {
        ...
        const { data, error } = await supabase.from('carbon_offsets').insert(batch).select();
  ...
  const downloadTemplate = () => {
    const sampleData = [
      {
        ...
        registry_id: "00000000-0000-0000-0000-000000000005",
        vintage: 2023,
        quantity_tco2e: 1000,
        price_per_ton: 15.50,
        status: "available",
    ...
    link.setAttribute("download", "carbon_offsets_template.csv");
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Bulk Carbon Offsets Upload</CardTitle>
        <CardDescription>
          Upload a CSV file with carbon offsets data.
        </CardDescription>
      </CardHeader>
...
export default BulkCarbonOffsetsUpload;
