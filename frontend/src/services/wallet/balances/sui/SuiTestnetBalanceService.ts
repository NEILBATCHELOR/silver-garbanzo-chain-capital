/**
 * Sui Testnet Balance Service
 * Fetches real balances and coin data from Sui testnet
 */
...
export class SuiTestnetBalanceService extends BaseChainBalanceService {
  constructor() {
    const config: BalanceServiceConfig = {
      chainId: 1, // Sui testnet (using 1 as identifier)
      chainName: 'Sui Testnet',
      name: 'Sui Testnet',
      symbol: 'SUI',
      decimals: 9,
      networkType: 'testnet',
      rpcUrl: import.meta.env.VITE_SUI_TESTNET_RPC_URL,
  ...
  protected async fetchNativeBalance(address: string): Promise<string> {
    if (!this.config.rpcUrl) {
      throw new Error('Sui Testnet RPC provider not configured');
    }
    ...
    try {
      ...
      console.warn(`⚠️ Sui Testnet balance fetch failed:`, error.message);
      throw error;
    }
  }
  ...
  protected async fetchTokenBalancesImpl(address: string): Promise<TokenBalance[]> {
    ...
    try {
      ...
      if (response.error) {
        console.warn(`⚠️ Sui Testnet coin fetch failed:`, response.error.message);
        return tokens;
      }
      ...
      for (const coinBalance of coinBalances) {
        try {
          ...
          if (balance > 0) {
            ...
            console.log(`🌊 [Testnet] ${tokenMetadata.symbol}: ${balanceFormatted} ($${(parseFloat(balanceFormatted) * tokenPrice).toFixed(2)})`);
          }
        } catch (tokenError) {
          console.warn(`⚠️ Failed to process Sui Testnet coin:`, tokenError.message);
        }
      }
    } catch (error) {
      console.warn(`⚠️ Sui Testnet coin enumeration failed:`, error.message);
    }
  ...
  private async getTokenMetadata(coinType: string): Promise<{ symbol: string; name: string; decimals: number }> {
    // Common Sui testnet coins - same as mainnet but could be different test tokens
    const knownCoins: { [key: string]: { symbol: string; name: string; decimals: number } } = {
      '0x2::sui::SUI': { symbol: 'SUI', name: 'Sui', decimals: 9 },
      '0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN': { symbol: 'USDC', name: 'USD Coin (Testnet)', decimals: 6 },
      '0xc060006111016b8a020ad5b33834984a437aaa7d3c74c18e09a95d48aceab08c::coin::COIN': { symbol: 'USDT', name: 'Tether (Testnet)', decimals: 6 }
    };
    ...
    return knownCoins[coinType] || { 
      ...
      name: `${extractedSymbol} Token (Testnet)`, 
      decimals: 9 
    };
  }
}

export const suiTestnetBalanceService = new SuiTestnetBalanceService();
