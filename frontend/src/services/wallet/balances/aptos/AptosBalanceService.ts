/**
 * Aptos Mainnet Balance Service
 * Fetches real balances and coin data from Aptos mainnet
 */
...
export class AptosBalanceService extends BaseChainBalanceService {
  constructor() {
    const config: BalanceServiceConfig = {
      chainId: 1, // Aptos mainnet
      chainName: 'Aptos',
      name: 'Aptos', // Alias for chainName
      symbol: 'APT',
      decimals: 8,
      networkType: 'mainnet',
  ...
  protected async fetchNativeBalance(address: string): Promise<string> {
    if (!this.config.rpcUrl) {
      throw new Error('Aptos RPC provider not configured');
    }
    ...
    try {
      ...
      console.warn(`⚠️ Aptos balance fetch failed:`, error.message);
      throw error;
    }
  }
    ...
    try {
      ...
      for (const coinStore of coinStores) {
        try {
          ...
          console.warn(`⚠️ Failed to process Aptos coin:`, tokenError.message);
        }
      }
    } catch (error) {
      console.warn(`⚠️ Aptos coin enumeration failed:`, error.message);
    }
  ...
  private async getTokenMetadata(coinType: string): Promise<{ symbol: string; name: string; decimals: number }> {
    ...
    const knownCoins: { [key: string]: { symbol: string; name: string; decimals: number } } = {
      ...
      '0xf22bede237a07e121b56d91a491eb7bcdfd1f5907926a9e58338f964a01b17fa::asset::USDC': { symbol: 'USDC', name: 'USD Coin', decimals: 6 },
      '0xf22bede237a07e121b56d91a491eb7bcdfd1f5907926a9e58338f964a01b17fa::asset::USDT': { symbol: 'USDT', name: 'Tether', decimals: 6 }
    };

    return knownCoins[coinType] || { 
      symbol: `COIN-${coinType.slice(2, 10)}`, 
      name: 'Unknown Coin', 
      decimals: 8 
    };
  }
